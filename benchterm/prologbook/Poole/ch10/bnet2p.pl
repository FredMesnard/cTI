% BAYESIAN NETWORK INTERPRETER
% Copyright D. Poole, 1996, based on VE algorithm
% of Zhang and Poole. This prunes irrelevant variables.

% A Bayes net is represented with the relations
...
% children(X,Cn) Cn is list of children of X
...
create_tree([Var|CVars],Vars,SomeX,X,Context,CTree) :-
   values(Var,Vals),
   create_tree_each_val(Vals,CVars,Vars,SomeX,X,Context,CTree).

% create_tree_each_val(Vals,CVars,Vars,SomeX,X,Context,CTree).
% creates a tree for each value in Vals.
create_tree_each_val([],_,_,_,_,_,[]).
create_tree_each_val([Val|Vals],CVars,Vars,
...
% computes product of the SomeX factors given X=Val, Vars=CVals
...
% reverse(L,R) is true if R contains same elments as list L, in reverse order
reverse(L,R) :-
   rev(L,[],R).
rev([],R,R).
rev([H|T],Acc,R) :-
...
% remove(E,L,R) true if R is the list L with one occurrence of E removed
remove(E,[E|L],L).
remove(E,[A|L],[A|R]) :-
   remove(E,L,R).

% remove_if_present(E,L,R) true if R is the list L with one occurrence of E removed
remove_if_present(_,[],[]).
