...
% This program does not assume built-in delaying, but has to delay
...
%   add_to_agenda(B,GG,[],BA),    % agenda is queue
%   append(R,BA,NR),
   select(P,NR,A).
...
%    updating the delayed constraints from DIC0 to DIC1

% CASE 0: inequality goal
solve_goal(goal((X \= Y),_),Pl,Pl,Ag,Ag,DB,DB,DIC0,DIC1) :-
    !,
