...
test(time-T):-statistics(runtime,_),chat_parser,statistics(runtime,[_,T]).
...
o_p_e_n(B,B,C,x(gap,nonterminal,close,C)).
...
quote(there).
quote(of).
quote('quote').		% use ` instead of ' to help assembler
quote(',').	
quote(s).
quote(than).
quote(at).
...
%go(Mes):-test(T),nl,write(Mes=[T]),nl.
